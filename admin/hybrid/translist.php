...
if($action =="download"){
	...
	header("Content-disposition: attachment; filename=registration.csv");
        ...
        $Receipt = new Receipt;
        $rsUsers = $Receipt->getRegistrationsSorted("Name","",0,0,"");
        if (mysql_affected_rows()){
	        while ($row=mysql_fetch_assoc($rsUsers)){
		        ...
		        print stripslashes($row['ctransreceipt']).",";
		        print "\n";
	        }
        }
        exit;
	...
	header("Content-disposition: attachment; filename=refundedtransaction.csv");
        ...
        if (mysql_affected_rows()){
	        while ($row=mysql_fetch_assoc($rsUsers)){
		        ...
		        print stripslashes($row['FormattedDate']).",";
		        print stripslashes($row['ctransreceipt']).",";
		        print stripslashes($row['ctransaction']).",";
		        print stripslashes($row['ctranspaymentmethod']).",";
		        print stripslashes($row['ctransamount']);
		        print "\n";
	        }
        }
        exit;
	...
	global $_OrderBy;global $_SortBy;global $searchterm;
        ...
        if (mysql_affected_rows()){
	        while ($row=mysql_fetch_assoc($rsUsers)){
		        ...
		        print '<td  align="center" align="center">'.$row['ctransaction'].'</td>';
		        print '<td  align="center" align="center">First time='.$row['FirstTimeActivation'].'<br>Recurrent ='.$row['RecurrentVerification'].'</td>';
