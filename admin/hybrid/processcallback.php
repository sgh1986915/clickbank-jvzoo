...
$CANCELPRODITEM = $cancelproditemrow['Value'];
	...
	if(isset($_GET['rfnd'])){
		if($_GET['rfnd']=="yes"){
			$Receipt = new Receipt();
			$Receipt->RemoveReceipt('YQVLEPDN');
		}
	}
...
function processtransaction(){
	...
	if( strlen(trim($PRODITEM))==0 || in_array($cproditem,$proditemarray)){
		emaildebugscript();
		if($ctransaction=="SALE" || $ctransaction=="BILL" || strtoupper($ctransaction)=="REBILL" || strtoupper($ctransaction)=="TEST_SALE" || strtoupper($ctransaction)=="TEST_BILL" || strtoupper($ctransaction)=="TEST" ){
